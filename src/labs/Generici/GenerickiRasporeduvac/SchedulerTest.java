package labs.Generici.GenerickiRasporeduvac;
...
class Timestamp<T> implements Comparable<Timestamp<T>> {
    ...
    public int compareTo(Timestamp<T> timestamp){
...
class Scheduler<T>{
    ...
    public Timestamp<T> next(){
        return timestamps.stream ()
                .filter (a ->a.getTime ().isAfter (LocalDateTime.now ()))
                .sorted ()
    ...
    public Timestamp<T> last(){
        return timestamps.stream ()
                .filter (a ->a.getTime ().isBefore (LocalDateTime.now ().minusSeconds (1)))
                .sorted ()
                .collect (Collectors.toList ())
                .get (0);
    }
    ...
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return timestamps.stream ().filter (t -> t.getTime ().isAfter (begin.minusSeconds (1)) && t.getTime ().isBefore (end.plusSeconds (1)))
                .collect (Collectors.toList ());
    }
