package Exercises.PrvKolokvium.Evaluator;
...
class EvaluatorBuilder{
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        // вашиот код овде
        if(type == Evaluator.TYPE.NO_CRIMINAL_RECORD) return a -> !a.hasCriminalRecord ();
        else if(type == Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE)
            return a -> !a.hasCriminalRecord () && a.getCreditScore () >= 500;
        else if(type == Evaluator.TYPE.MORE_CREDIT_SCORE) return a -> a.getCreditScore () >= 500;
        else if(type == Evaluator.TYPE.MORE_EXPERIENCE) return a -> a.getEmploymentYears () >= 10;
        else if(type == Evaluator.TYPE.MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE)
            return a -> a.getEmploymentYears () >= 10 && a.getCreditScore () >= 500;
        else if(type == Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE)
            return a -> !a.hasCriminalRecord () && a.getEmploymentYears () >= 10;
        else if(type == Evaluator.TYPE.INVALID) throw new InvalidEvaluation ();
        return null;
    }
}
